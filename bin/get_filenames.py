...
def run(metric_category):
    ...
    elif metric_category == 'pool':
        filenames = [
      ["neo4j.test2.pool.transaction.test2.free.csv", 0, ""],
      ["neo4j.test2.pool.transaction.test2.total_size.csv",0,""],
      ["neo4j.test2.pool.transaction.test2.total_used.csv",0,""],
      ["neo4j.test2.pool.transaction.test2.used_heap.csv",0,""],
      ["neo4j.test2.pool.transaction.test2.used_native.csv",0,""]
      ]
    ...
    elif metric_category == 'jvm_memory2':
        filenames = [
            ["neo4j.vm.file.descriptors.count.csv",0,"-mb"],
            ["neo4j.vm.file.descriptors.maximum.csv",0,"-mb"],
            ["neo4j.vm.heap.committed.csv",0,"-mb"],
            ["neo4j.vm.heap.max.csv",0,"-mb"],
            ["neo4j.vm.heap.used.csv",0,"-mb"],
            ["neo4j.vm.pause_time.csv",0,"-mb"],
            ["vm.memory.pool.code_cache.csv", 0, "-mb"]
        ]
    elif metric_category == 'file_descriptors':
        filenames = [
                ["neo4j.vm.file.descriptors.maximum.csv",0,""],
                ["neo4j.vm.file.descriptors.count.csv",0,""],
                ["neo4j.vm.file.descriptors.count.csv.final",0,""],
                ["neo4j.vm.file.descriptors.maximum.csv.final",0,""]
        ]
    else:
    ...
    for file in filenames:
       ...
       if not os.path.isfile(metrics_file):
            ...
            print_log.run("WARN","Missing/Skipping Metric File: " + metrics_file)
       else:
    ...
    if len(filenames_keep) == 0:
        print (" ")
        print ("WARN: No matching metric names found!   Please ensure correct dbname(" + globals.database + ") and/or  metric category(" + metric_category + ") has been provided." )
        print (" ")
