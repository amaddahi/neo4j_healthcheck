...
def initialize():
    ...
    elif os.path.isdir('metrics'):
        globals.metrics_directory = os.path.join(globals.working_directory, 'metrics')


    if (globals.metrics_directory != None ):
        if not (os.path.isdir(globals.metrics_directory) and fnmatch.filter(os.listdir(globals.metrics_directory), '*.csv')):
              print_log("Exiting:  Can not find any CSV metrics files in the metrics directory (" + globals.metrics_directory + ") in this folder")
              exit(1)
    else:
        print_log("""Exiting      :  Can not find a metrics directory: 

                1) in the current directory 
                2) nor under $NEO4J_HOME 
